...
# 1. Replace the Trucks Configuration expander block
old_expander = """        with st.expander("Trucks Configuration", expanded=False):
...
new_expander = """        with st.expander("Trucks Configuration", expanded=False):
...
# 2. Fix the initial_routes generation
old_routes_gen = """                rt_name = row['Rt']
...
old_routes_gen_fallback = """                t_name = row['Rt']
                if t_name in truck_name_to_idx:
                    t_idx = truck_name_to_idx[t_name]
                    # avoid consecutive duplicates
                    if not initial_routes[t_idx] or initial_routes[t_idx][-1] != node_id:
...
if old_routes_gen in content:
    content = content.replace(old_routes_gen, new_routes_gen)
elif old_routes_gen_fallback in content:
    content = content.replace(old_routes_gen_fallback, new_routes_gen)
else:
    print("Could not find routes gen block")
